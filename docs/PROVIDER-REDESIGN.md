...
| codex | Read-only, completed turn, exact resume | Commentary/final/retry/failure/JSONL EOF/output-file conflict fixtures | Two-turn acceptance-codex; reads work, writes denied, exact thread | Passed for pinned 0.154.0 profile 2; review passed; CI/merge pending |
| claude | Restricted reads, managed policy, final result, resume | Result/error/truncation/init tools/session/accounting fixtures | Two-turn acceptance-claude; reads/search work, forbidden effects absent | Planned |
