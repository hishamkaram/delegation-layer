...
The user approved the [supervised native metadata inspection amendment](NATIVE-METADATA-INSPECTION.md)
for Claude integration. It governs the new two-stage preparation boundary:
static candidate validation, a shared supervisor-owned native inspection before
ordinary task admission, and a fresh core-owned inspection inside the supervised
runner before paid launch. This replaces the in-process-only credential metadata
...
2. Assign implementation work with a finite brief containing the applicable specifications, verified findings, file ownership, interface contracts and acceptance criteria. Coordinate shared interfaces before concurrent edits. Root may implement directly or use bounded agents, and must retain substantive handoffs and actual test results. Delegated implementation agents use gpt-5.6-luna with max reasoning effort. Per the user’s updated instruction, agy is used only for provider integration and live acceptance tests, never implementation or review fixes. Native test calls require finite input, exact session identity, captured output and positive completion evidence.
...
| 7 | Duo Supervisor fan-out uses layer with skill unchanged | Shim compatibility, sidecar/exit/session/detach/recovery semantics, no duplicate turn | Unmodified review skill reaches actual layer task(s) via opt-in transport and collects verified result |
...
For early phases a live CLI means the newly built executable and real subprocess boundaries. Paid providers cannot be exercised through an adapter before it exists. From each adapter phase onward, phase-specific live targets are mandatory locally; missing auth/config/binary or inconclusive evidence fails acceptance. CI runs hermetic/unit and isolated supervisor CLI checks without provider credentials. Releases advertise the three adapter modes and their runtime capability requirements; cross-builds alone are labeled build targets.
...
Publish a capability matrix describing each adapter's required flags, policy checks and observed live environments. The runtime accepts any CLI version that passes executable, version, help and required-flag preflight. Baseline build targets are darwin/amd64, darwin/arm64, linux/amd64, linux/arm64. Native receipts state their actual architecture as an observation. Run the complete unit/race/config/gate suite, isolated pueue suite and each provider live target before tagging.
...
The following specifications are normative within their assigned phase. Where a future live measurement disproves a proposed flag/schema, stop that phase's advancement, record the counterexample, correct the brief and implementation through root Codex or the assigned Luna/max agent, and rerun acceptance. Do not treat the untested recipe itself as evidence that its behavior works.
...
- Standard library plus pinned `golang.org/x/sys v0.48.0` for Unix locks/full flush and pinned `go.yaml.in/yaml/v3 v3.0.5` solely for strict pueue configuration parsing (Phase 2 amendment; see `docs/PHASE-2-WORKPLAN.md`). This pin compiled and ran on Go 1.27.1/macOS arm64; the Linux amd64 variant cross-compiled. Keep `go.mod`/`go.sum`. Do not add a storage abstraction, general event bus or provider service locator.
...
This supplements the historical CLI help/version captures and official citations in `provider-checks/REPORT.md`. Those captures document the flags and output shapes used to build the adapters; they do not constrain installed releases. Preparation discovers each executable, accepts any nonempty reported version, checks its help output for every adapter flag, and then binds the observed executable identity to the task. No additional model or effort override is required for these phases.
...
| codex:exec | read-only | never ask; no approval escalation | workspace-write behavior, unrestricted execution |
...
Declare each adapter's launch arguments, output predicate and policy checks in code. Ordinary dispatch performs runtime executable/help capability preflight, validates its effective configuration and records observed identity; it does not require a checked-in receipt or run a paid probe. A missing executable, non-executable path, failed version/help command or missing flag fails before supervisor admission. Unsupported permission intents fail before admission, and `--allow-unsupported` cannot bypass safety checks or nonempty raw argv rejection.

### Effective configuration and launch plan

A provider-specific resolver produces a typed launch plan before admission: absolute executable, exact argv, canonical workspace, finite stdin file, required environment changes, settings-source inventory, non-secret normalized policy digest, binary version, model/effort request, and expected session identity when resuming. Do not expose credentials in metadata or config snapshots. Preserve supported native authentication stores; construct child and supervisor environments from an explicit non-secret allowlist rather than forwarding ambient credential variables. Keep secrets out of command lines and supervisor task records.

Resolve all relevant sources for that provider and discovered CLI, including platform/managed sources that still apply after the chosen suppression flags. Validate the fields affecting tool availability, writable roots, bypass rules, hooks, MCP and custom executables. Benign appearance settings do not need rejection; unknown values in policy-affecting fields do. An unreadable applicable source, unrecognized policy mechanism, or conflict yields `unsupported-effective-config` before launch, with the source path/type and reason rather than secret contents.

Re-resolve just before provider launch. If the normalized policy digest or workspace identity differs from admission, refuse this launch with explicit evidence; do not silently adapt. Persist the policy digest and relevant normalized decisions. This catches ordinary changes and makes the check auditable; a fingerprint is not an OS isolation primitive. On resume, resolve again rather than inheriting the predecessor's effective policy.
...
The current candidate explicitly registers exactly the canonical task workspace with `--add-dir` and selects `--mode accept-edits`, including on continuation. It adds no other workspace root. Earlier candidates failed their workspace read or denied file creation. The per-run edit mode is documented to approve file operations while retaining shell permission rules. The sandbox, non-workspace policy, source validation, and outside-write control remain mandatory; this candidate requires its own live safety run before release.

The brief is stdin, closed after its bytes. Piped input selects headless mode on the discovered CLI, as the existing project measurement demonstrated. Do not append `-p <brief>` or an empty positional prompt. Resume adds exactly `--conversation <recorded-conversation-id>` and otherwise uses the same profile. If a discovered CLI needs an explicit print switch with stdin, establish that exact spelling in acceptance and record it; never guess by launching a second task.
...
Preflight reads the applicable non-secret policy from the documented global settings location and every additional project/agent/workspace source exposed by the discovered CLI. The CLI exposes no per-run settings-file flag in the historical help capture. Do not manufacture one, rewrite global settings, or repurpose HOME. The baseline works with an already-compatible validated provider configuration.
...
On continuation, require conversation_id equal to the recorded predecessor ID. Treat num_turns as cumulative metadata, not the number of tasks launched. Store usage and duration as conversation-cumulative unless output-contract fixtures establish otherwise for this mode. Timed-out or incomplete usage is unreliable even when every counter is zero. Do not parse Antigravity's private conversation database in v1; expose an unavailable transcript descriptor where no supported lookup exists. Raw task evidence remains recoverable.
...
Malformed/unsafe settings are tested with injected resolver fixtures or a verified temporary provider configuration mechanism; never alter global settings for testing. Failure to produce the historical empty-SUCCESS timeout shape on an installed CLI is not by itself a defect: record the actual timeout envelope and require correct non-publication for it while retaining the old regression fixture.
...
The Codex candidate uses the bounded personal-file-auth and authoritative macOS preference checks documented in [Codex acceptance](CODEX-ACCEPTANCE.md). Other authentication or managed-policy profiles fail before admission. The explicit hook/plugin feature gates and file-store override are part of the profile, and require exact argv, policy and native evidence before a task can start.
...
`make acceptance-codex` performs fixtures and **two bounded provider turns**: (1) read a nonce from a scratch workspace and attempt writes to workspace/sibling sentinels; no sentinel may change, successful Read must be observed, and no permission host may be consulted; (2) resume the exact resulting thread with a new task, recover the nonce, verify the identity and immutable original result. Workspace-write and arbitrary existing-session resume remain unsupported until separately implemented and tested.
...
Live acceptance uses the caller's already signed-in native account. Claude's
macOS Keychain selection is tied to that account, so the harness keeps the real
`HOME`/Keychain context and relocates temporary, cache, log, state, and
workspace paths to task-owned roots. It never copies or opens credential files,
and it never changes Keychain entries or search-list settings. The native
supervisor inspection still verifies the account immediately before admission
and before launch; the plaintext fallback is recorded with metadata only and
does not block a normal account. `--safe-mode` and `--restricted` disable
unmanaged user/project/local settings while managed policy remains validated.
The receipt records that authentication used the host login context, so it is
not misrepresented as a credential-store isolation proof.

SessionStart evidence must confirm the actual tools/permission mode wherever the CLI schema reports them; unexpected mutating tools or a changed mode mean no publication and a task-start refusal. Preflight performs the protection before startup; checking init afterward is additional evidence, not a way to undo startup hooks.
...
Evidence level: command availability and documented semantics were checked; runtime capability is established only by the executable/help preflight and the relevant unit/live tests. No release status or provider-version acceptance is produced by this document.
...
- Safety checks apply to agent-initiated tool effects, excluding ordinary native auth/session/log/cache writes. Each enabled permission mapping must have controlled unit/live evidence for its effective policy and containment behavior. Build targets may precede live observations; the capability table distinguishes those observations without pinning a provider release.
