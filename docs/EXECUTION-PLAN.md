...
The root Codex agent is orchestrator and reviewer. The installed agy CLI performs implementation and every review fix. Other agents can investigate, specify, and review; they do not substitute for agy as the implementer. The user's authorization includes branch/worktree creation, normal commits/pushes, PR creation, waiting for CI, and squash merging only after acceptance and review pass.
...
2. Send agy a finite file-backed brief containing the applicable specifications and prior review findings. Use the real installed CLI, finite stdin then EOF, explicit print timeout, structured stdout, separate stderr, a unique attempt prefix and recorded conversation ID. Never start a replacement merely because the watcher detached or output is incomplete. Observe/recover the current turn; new turns are explicitly recorded.
3. agy implements only that phase and runs its checks. It must not merge, publish releases, alter remote visibility, signal process IDs/groups, modify global provider settings, or start later phases. Review fixes return to the exact workspace and explicit conversation when continuation is available.
4. Freeze the proposed head for review. Root independently reads the entire changed surface and relevant consumers, runs meaningful unit/integration tests, and executes the phase's real CLI acceptance. Obtain an independent read-only review using the two-model workflow. Every accepted finding has a concrete trigger, code citation, correction and regression test. Verify disputed findings; do not count agreement as proof.
5. Send all unresolved findings to agy. Re-run affected checks and re-review the changed head. Repeat until no unresolved actionable review findings remain. Do not satisfy this condition by relabeling a defect, skipping tests, suppressing diagnostics, or manufacturing terminal output.
...
7. Wait for all required GitHub checks on the exact proposed head to succeed and inspect any failure. Return fixes to agy, push, re-review and wait again. Never merge on a stale green SHA, while checks are pending/skipped unexpectedly, or when required live receipts are missing.
...
This is a finite phase sequence, not a scheduled background automation. Progress and the active agy attempt are recorded so a conversation compaction does not restart paid work.
