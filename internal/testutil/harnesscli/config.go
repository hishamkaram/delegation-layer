...
const (
	fixtureConfigName = "provider-fixture.json"
	maxHookDelay      = 10 * time.Second
	maxConfigBytes    = task.MaxControlRecordSize
)

var errInvalidHarnessReference = errors.New("invalid supervisor harness reference")
...
// HookConfig is the bounded hook portion of provider-fixture.json.
...
func canonicalExecutable(path string) (string, error) {
	if path == "" {
		...
		if err != nil {
			return "", fmt.Errorf("resolving supervisor executable: %w", err)
		}
	}
	if !filepath.IsAbs(path) {
		return "", fmt.Errorf("supervisor executable must be absolute: %w", errInvalidHarnessReference)
	}
	...
	if filepath.Clean(resolved) != clean {
		return "", fmt.Errorf("supervisor executable must be canonical: %w", errInvalidHarnessReference)
	}
	...
	if !info.Mode().IsRegular() || info.Mode().Perm()&0o111 == 0 {
		return "", fmt.Errorf("supervisor executable is not executable: %w", errInvalidHarnessReference)
	}
	return clean, nil
}
...
func loadHarnessConfig(path string) (loadedHarnessConfig, error) {
	...
	if err != nil {
		return loadedHarnessConfig{}, fmt.Errorf("resolving supervisor fixture config: %w", err)
	}
	if filepath.Clean(resolved) != path {
		return loadedHarnessConfig{}, fmt.Errorf("supervisor fixture config must be canonical: %w", errInvalidHarnessReference)
	}
	...
	if err = task.DecodeStrict(data, &cfg); err != nil {
		return loadedHarnessConfig{}, fmt.Errorf("decoding supervisor fixture config: %w", err)
	}
...
func validateHarnessConfig(cfg HarnessConfig) error {
	if cfg.SchemaVersion != 1 {
		return fmt.Errorf("unsupported supervisor fixture schema version %d", cfg.SchemaVersion)
	}
	...
	if cfg.Hooks.DelayMS < 0 || cfg.Hooks.DelayMS > maxHookDelay.Milliseconds() {
		return errors.New("supervisor fixture hook delay exceeds the finite bound")
	}
	...
	default:
		return fmt.Errorf("unsupported supervisor fixture hook mode %q", cfg.Hooks.Mode)
	}
	if cfg.Hooks.ReleasePath != "" {
...
func validateEnvironment(environment []string) error {
	if environment == nil {
		return errors.New("supervisor fixture environment must be an explicit array")
	}
	seen := make(map[string]bool)
	for _, value := range environment {
		key, _, found := strings.Cut(value, "=")
		if !found || key == "" || strings.ContainsRune(value, '\x00') || seen[key] {
			return errors.New("supervisor fixture environment contains an invalid or duplicate entry")
		}
		seen[key] = true
	}
	return nil
...
func validateCleanAbsolute(path string) error {
	if !filepath.IsAbs(path) || filepath.Clean(path) != path {
		return fmt.Errorf("supervisor fixture path must be clean and absolute: %w", errInvalidHarnessReference)
	}
	return nil
}
...
func validateEventsDirectory(path string) error {
	...
	if err != nil {
		return fmt.Errorf("resolving supervisor events directory: %w", err)
	}
	if canonical != path {
		return fmt.Errorf("supervisor events directory must be canonical: %w", errInvalidHarnessReference)
	}
	for current := path; ; current = filepath.Dir(current) {
		info, statErr := os.Lstat(current)
		if statErr == nil {
			if info.Mode()&os.ModeSymlink != 0 {
				return fmt.Errorf("supervisor events directory contains a symlink: %w", errInvalidHarnessReference)
			}
			if !info.IsDir() {
				return fmt.Errorf("supervisor events directory component is not a directory: %w", errInvalidHarnessReference)
			}
		} else if !errors.Is(statErr, os.ErrNotExist) {
			return fmt.Errorf("checking supervisor events directory: %w", statErr)
		}
...
func readRegular(path string, limit int64) (data []byte, err error) {
	...
	if err != nil {
		return nil, fmt.Errorf("opening supervisor fixture file: %w", err)
	}
	...
	if !info.Mode().IsRegular() {
		return nil, fmt.Errorf("supervisor fixture file must be regular: %w", errInvalidHarnessReference)
	}
...
func hashRegular(path string) (digest string, err error) {
	f, err := os.OpenFile(path, os.O_RDONLY|unix.O_NOFOLLOW|unix.O_NONBLOCK, 0)
	if err != nil {
		return "", fmt.Errorf("opening supervisor provider executable: %w", err)
	}
	defer func() { err = errors.Join(err, f.Close()) }()
	info, err := f.Stat()
	if err != nil {
	...
	if !info.Mode().IsRegular() || info.Mode().Perm()&0o111 == 0 {
		return "", fmt.Errorf("supervisor provider executable must be executable regular file: %w", errInvalidHarnessReference)
	}
	h := sha256.New()
