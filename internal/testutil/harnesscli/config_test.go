...
func TestPrepareProfileReadsAndBindsAllFixtureDigests(t *testing.T) {
	...
	request := task.TaskRecord{TaskID: fixture.provider.TaskID, Provider: fakeprovider.Provider, Mode: fakeprovider.Mode, CanonicalCwd: dir}
...
func TestPrepareProfileRejectsNativeTimeoutBeforeReadingConfig(t *testing.T) {
	...
	request := task.TaskRecord{
		...
		Mode:            fakeprovider.Mode,
		RequestedConfig: task.TaskConfig{Permission: fakeprovider.Mode, NativeTimeout: "1s"},
	}
...
func TestNewMainKeepsRecoveryIndependentFromMissingConfig(t *testing.T) {
	...
	if _, err := m.Dependencies().PredicateRegistry().Resolve(fakeprovider.Predicate().Reference()); err != nil {
		t.Fatal(err)
	}
	request := task.TaskRecord{TaskID: "0123456789abcdef0123456789abcdef", Provider: fakeprovider.Provider, Mode: fakeprovider.Mode}
...
func testHarnessFixture(t *testing.T, dir string) harnessFixture {
	...
	providerConfig := fakeprovider.ProviderConfig{Scenario: "success", ArtifactDir: filepath.Join(dir, "artifacts"), LifetimeMS: 100, TaskID: "0123456789abcdef0123456789abcdef", SessionID: "conv-1", Argv: []string{"$(literal)", "value;token"}}
