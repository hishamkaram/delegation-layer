// Package harnesscli composes the acceptance-only command wrappers. It is not
...
import (
	...
	"github.com/hishamkaram/delegation-layer/internal/testutil/fakeprovider"
)

const (
	publisherVersion       = "fixture-v2"
	invalidSupervisorPath  = "/__supervisor_fixture_invalid__/supervisor"
...
func (m *Main) prepareProfile(request task.TaskRecord) (app.PreparedProfile, error) {
	if request.Provider != fakeprovider.Provider || request.Mode != fakeprovider.Mode {
		return app.PreparedProfile{}, fmt.Errorf("%w: fixture profile requires %s/%s", task.ErrIdentityMismatch, fakeprovider.Provider, fakeprovider.Mode)
	}
	...
	if providerDigest != loaded.Config.ProviderSHA256 {
		return app.PreparedProfile{}, errors.New("supervisor provider executable digest mismatch")
	}
	...
	if task.ComputeSHA256(configData) != loaded.Config.ProviderConfigSHA256 {
		return app.PreparedProfile{}, errors.New("supervisor provider config digest mismatch")
	}
	providerConfigValue, err := fakeprovider.ParseProviderConfig(configData)
	if err != nil {
		return app.PreparedProfile{}, err
	}
	...
	return app.PreparedProfile{
		...
		Identity: func(expected task.SessionExpectation, record func(task.SessionIdentity) error) (execution.IdentityObserver, error) {
			return fakeprovider.NewIdentityObserver(request.TaskID, expected, record)
		},
	}, nil
}
...
func canonicalFileReference(path string) (string, error) {
	...
	if filepath.Clean(resolved) != path {
		return "", fmt.Errorf("supervisor path must be canonical: %w", errInvalidHarnessReference)
	}
	return path, nil
}
...
func buildAcceptanceRegistry() predicate.Registry {
	fixtureInterpreter, err := predicate.Default().Resolve(task.FixturePredicateRef())
	if err != nil {
		panic(err)
	}
	registry, err := predicate.New(fixtureInterpreter, fakeprovider.Predicate())
	if err != nil {
		panic(err)
	}
	return registry
