...
func buildCompiledFake(t *testing.T, output string) {
	...
	if !ok {
		t.Fatal("locating fakesupervisor source")
	}
	...
	cmd := exec.Command(goTool, "build", "-buildvcs=false", "-o", output, "./internal/testutil/fakesupervisor/cmd/pueue")
	cmd.Dir = repo
	if outputData, err := cmd.CombinedOutput(); err != nil {
		t.Fatalf("build compiled fake supervisor: %v: %s", err, outputData)
	}
}
