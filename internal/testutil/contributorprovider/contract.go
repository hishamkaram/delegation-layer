...
const contractV1 = `{"adapter":"synthetic:contributor-proof","mode":"read-only","version":"1","protocol":"contributor-proof/v1","envelope":{"fields":"protocol,task_id,session_id,status,answer; one strict JSON object; no unknown or duplicate fields","status":"complete|rejected","answer":"UTF-8 string","max_bytes":1048576},"brief":{"fields":"case,answer?,nonce?","stdin":"one bounded strict JSON object followed by EOF","max_bytes":174677},"identity":"session_id is session-[0-9a-f]{32}; envelope task_id equals seal task_id; session_id equals expected and recorded session when supplied","success":"status=complete,exit_code=0,answer non-whitespace; absent answer.txt uses envelope answer; present answer.txt must be nonempty and byte-identical","failure":"rejected status or nonzero exit; empty or conflicting answer.txt rejects; malformed envelope and identity mismatch reject","runtime":"create-once nonsecret launch/completion receipts and private per-session remembered nonce or answer file"}` + "\n"
