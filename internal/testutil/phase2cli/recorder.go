...
var (
	errRecorderDuplicate = errors.New("phase2 event recorder received a duplicate event")
	errWaitHook          = errors.New("phase2 wait hook failure")
	errCaptureHook       = errors.New("phase2 capture hook failure")
)
...
func startFailed(cmd *exec.Cmd) error {
	cmd.Path = filepath.Join(filepath.Dir(cmd.Path), ".phase2-fixture-provider-absent")
	return cmd.Start()
}
