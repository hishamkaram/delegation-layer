...
func readPolicySource(path string) (sourceBytes, error) {
	...
	if err != nil || canonical != path || filepath.Clean(path) != path {
		return result, fmt.Errorf("%w: policy source must have a canonical path: %s", ErrUnsupportedProfile, path)
	}
	...
	if err != nil {
		return result, fmt.Errorf("%w: cannot inspect policy source %s: %w", ErrUnsupportedProfile, path, err)
	}
	if !before.Mode().IsRegular() {
		return result, fmt.Errorf("%w: policy source is not a regular file: %s", ErrUnsupportedProfile, path)
	}
	data, err := readStablePolicyFile(path, before)
	if err != nil {
		return result, fmt.Errorf("%w: cannot read stable policy source %s: %w", ErrUnsupportedProfile, path, err)
	}
...
func readStablePolicyFile(path string, before os.FileInfo) (data []byte, resultErr error) {
	...
	if err = checkPolicyFileAfterRead(path, f, opened, int64(len(data))); err != nil {
		return nil, err
	}
	return data, nil
}

func checkPolicyFileAfterRead(path string, f *os.File, opened os.FileInfo, length int64) error {
	final, err := f.Stat()
	if err != nil {
		return err
	}
